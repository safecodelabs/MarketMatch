...
const { sendMessage, sendButtons, sendLanguageList, sendList } = require("../services/messageService");
...
async function handleIncomingMessage(sender, msgBody, metadata = {}) {
  ...
  if (metadata?.interactive && metadata.interactive.type === "list_reply") {
...
if (greetings.includes(lowerMsg)) {
  ...
  if (isNewUser) {
    ...
    await sendList(
      ...
      "Choose an option:",
      "",
      menuRows.length ? [{ title: "Menu", rows: menuRows }] : []
    );
    return session;
  }
  ...
  // -----------------------------------------
  // MAIN MENU HANDLERS (LIST MESSAGE IDs)
  // -----------------------------------------
